...
<div class="max-w-4xl mx-auto">
    ...
    <div class="animated-card card-hover bg-white rounded-xl shadow-lg p-8 border-2 border-gray-300 animate-delay-200">
        <form action="{{ route('admin.users.roles.update', $role) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="border-t border-gray-300 pt-6">
                ...
                @if($permissions->isEmpty())
                    <div class="bg-gray-50 border-2 border-gray-300 rounded-lg p-6 text-center">
                        ...
                        <p class="text-gray-500 font-semibold">No permissions available.</p>
                    </div>
                @else
                    @php
                    ...
                    @foreach($permissions as $group => $groupPermissions)
                        <div class="mb-6 permission-group animated-card" data-group="{{ $group }}">
                            <div class="flex items-center justify-between mb-3">
                                <h4 class="font-bold text-gray-800 capitalize text-lg flex items-center">
                                ...
                                <div class="flex gap-2">
                                    <button type="button" onclick="selectAllInGroup('{{ $group }}')" 
                                        class="text-xs px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 font-semibold transition-all duration-200 flex items-center">
                                    ...
                                    <button type="button" onclick="deselectAllInGroup('{{ $group }}')" 
                                        class="text-xs px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-semibold transition-all duration-200 flex items-center">
                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="border-2 border-gray-300 rounded-lg bg-white shadow-sm" style="max-height: 200px; overflow-y: auto;">
                                <div class="p-3 space-y-2">
                                    @foreach($groupPermissions as $permission)
                                        <label class="flex items-start p-3 rounded-lg hover:bg-blue-50 cursor-pointer transition-all duration-200 border border-transparent hover:border-blue-200">
                                            <input type="checkbox" 
                                                name="permissions[]" 
                                                value="{{ $permission->id }}"
                                                id="perm-{{ $group }}-{{ $permission->id }}"
                                                class="permission-checkbox-{{ $group }} mt-1 rounded border-2 border-gray-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500 w-4 h-4 cursor-pointer"
