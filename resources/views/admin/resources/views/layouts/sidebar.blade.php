...
<aside id="sidebar"
    class="fixed flex flex-col mt-0 top-0 px-5 left-0 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-40 border-r border-gray-200"
    x-data="{
        openSubmenus: {},
        init() {
        ...
        isActive(path) {
            ...
            // For top-level items: active for exact or nested paths, e.g. admin/positions or admin/positions/5
    ...
    <!-- Logo Section -->
    <div class="pt-9 pb-8 flex"
        :class="(!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen) ?
        'xl:justify-center' :
        'justify-start'">
        <a href="{{ route('admin.dashboard') }}">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="dark:hidden h-16 w-auto object-contain" src="{{ asset('image/logo.png') }}" alt="{{ config('app.name') }}" />
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="hidden dark:block h-16 w-auto object-contain" src="{{ asset('image/logo.png') }}" alt="{{ config('app.name') }}" />
            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                class="h-12 w-12 object-contain" src="{{ asset('image/logo.png') }}" alt="{{ config('app.name') }}" />
        </a>
    </div>

    <!-- Navigation Menu + Bottom Logout (fills full height so logout stays at bottom) -->
    <div class="flex-1 flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar">
                                ...
                                <li>
                                        ...
                                        <button @click="toggleSubmenu({{ $groupIndex }}, {{ $itemIndex }})"
                                            ...
                                            <span class="sidebar-icon"
                                                :class="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) ?
                                                    'sidebar-icon-active' : 'sidebar-icon-inactive'">
                                    ...
                                    @else
                                        ...
                                        <a href="{{ $item['path'] }}" class="menu-item group"
                                            ...
                                            <!-- Icon -->
                                            <span class="sidebar-icon"
                                                :class="isActive('{{ $item['path'] }}') ? 'sidebar-icon-active' :
        ...
        <div x-data x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen" x-transition class="mt-auto">
            @include('tailadmin::layouts.sidebar-widget')
        </div>
